...
// Ensure import for defaultTpBahasaInggris is present
if (!code.includes('defaultTpBahasaInggris')) {
  code = code.replace(
...
const newLogic = `
    ...
    if (!isPancasila && !isInggris) {
      showNotif("Maaf, muat TP otomatis saat ini baru tersedia untuk Pendidikan Pancasila & Bahasa Inggris.", "error");
      return;
    }
...
// we need to replace everything between `if (!mapelObj) return;` and `let indexCounter = 0;`
code = code.replace(/if \(!mapelObj\.nama\.toLowerCase\(\)\.includes\('pancasila'\)\) \{[\s\S]*?const tpsToInject = kelasData\[sem\];\s*if \(!tpsToInject \|\| tpsToInject\.length === 0\) \{\s*showNotif\(\`Maaf, data TP default untuk Kelas \$\{parsedKelas\} Semester \$\{sem\} belum tersedia\.\`, "error"\);\s*return;\s*\}/, newLogic);
